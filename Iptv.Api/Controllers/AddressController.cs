...
[ApiController]
[Route("v1/address/")]
public class AddressController(IAddressHandler addressHandler) : ControllerBase
{
    [HttpGet("get")]
    public async Task<IActionResult> GetAdressByUserId()
    {
        try
        {
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;

            var result = await addressHandler.GetAddress(userId);
    ...
    [HttpPut("update")]
    public async Task<IActionResult> UpdateAddressByUserId(Address address)
    {
        try
        {
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            
            var result = await addressHandler.UpdateAddress(userId, address);
